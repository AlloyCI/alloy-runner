...
AlloyCI Runner can use Kubernetes to run builds on a kubernetes cluster. This is
possible with the use of the **Kubernetes** executor.

The **Kubernetes** executor, when used with AlloyCI CI, connects to the Kubernetes
API in the cluster creating a Pod for each AlloyCI CI Job. This Pod is made
up of, at the very least, a build container and an additional container for each
`service` defined by the AlloyCI CI yaml. The names for these containers
are as follows:
...
1. **Build**: User build.
1. **Post-build**: Create cache, upload artifacts to AlloyCI. This also uses
...
If you are running the AlloyCI Runner within the Kubernetes cluster you can omit
...
Additionally, Kubernetes namespace can be overwritten on `.alloy-ci.json` file, by using the variable
...
Additionally, Kubernetes service account can be overwritten on `.alloy-ci.json` file, by using the variable
...
Additionally, Kubernetes pod annotations can be overwritten on the `.alloy-ci.json` file, by using `KUBERNETES_POD_ANNOTATIONS_*` for variables and `key=value` for the value. The pod annotations will be overwritten to the `key=value`. Also, multiple annotations can be applied. For example:
``` yaml
 variables:
...
Each of the keywords can be defined in the `config.toml` for the alloy runner.
...
as a `service` usually in your `.alloy-ci.json` it will be run as a separate
...
There are a couple of caveats when using docker in your builds while running on
a kubernetes cluster. Most of these issues are already discussed in the
[**Using Docker Build**](https://docs.alloy.com/ce/ci/docker/using_docker_build.html)
section of the alloy-ci
documentation but it is worth it to revisit them here as you might run into
some slightly different things when running this on your cluster.
